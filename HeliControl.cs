...
{
    [Info("HeliControl", "Shady", "1.3.4", ResourceId = 1348)]
    ...
    class HeliControl : RustPlugin
    {
        ...
        void OnServerInitialized()
        {
            ...
            {
                var eventPrefabs = GameObject.FindObjectsOfType<TriggeredEventPrefab>();
        ...
        void OnEntitySpawned(BaseNetworkable entity)
        {
            ...
            if (prefabname.Contains("rocket_heli"))
            {
                ...
                if (explosion == null || explosion.IsDestroyed || explosion.gameObject == null) return;
                ...
                else
                {
                    if (MaxHeliRockets > 12 && ownerID == 0)
                    {
                        ...
                        foreach(var heli in BaseHelicopters)
                        {
                            if (heli == null || heli.IsDestroyed || heli.gameObject == null) continue;
                        ...
                        if (curCount >= 12)
                        {
                            ...
                            if (heliAI == null || heliAI.gameObject == null) return;
                            var actCount = 0;
                            Action fireAct = null;
                            fireAct = new Action(() =>
                            {
        ...
        object CanPlayerCallHeli(BasePlayer player, bool ch47 = false)
        {
            ...
            if (!ch47 && (BaseHelicopters.Count + 1) > MaxActiveHelicopters) return string.Format(GetMessage("tooManyActiveHelis", player.UserIDString));
            return null;
        }
        ...
        private void cmdCallToPlayer(BasePlayer player, string command, string[] args)
        {
            ...
            try
            {
                ...
                if (args.Length == 0)
                {
                    ...
                    if (newHeli != null && !permission.UserHasPermission(player.UserIDString, "helicontrol.dropcrates")) newHeli.maxCratesToSpawn = 0;
        ...
        private void consoleCallHeli(ConsoleSystem.Arg arg)
        {
            ...
            var noDrop = (player != null) ? !permission.UserHasPermission(player.UserIDString, "helicontrol.dropcrates") : false;
        ...
        CooldownInfo GetCooldownInfo(ulong userId)
        {
        ...
        private BasePlayer FindPlayerByPartialName(string name, bool sleepers = false, params BasePlayer[] ignore)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < BasePlayer.activePlayerList.Count; i++)
                {
                    ...
                    var cleanName = CleanPlayerName(pName);
                    if (!string.IsNullOrEmpty(cleanName)) pName = cleanName;
                    if (pName.IndexOf(name, StringComparison.OrdinalIgnoreCase) >= 0) matches.Add(p);
                }
                if (sleepers)
                {
                ...
                if (topMatch != null && topMatch.Count() > 0)
                {
        ...
        int Fitness(string individual, string target)
        {
            var val = Enumerable.Range(0, Math.Min(individual.Length, target.Length)).Count(i => individual[i] == target[i]);
            return val;
        }


        int ExactMatch(string comp1, string comp2, StringComparison options = StringComparison.CurrentCulture)
        {
        ...
        string CleanPlayerName(string str)
        {
            if (string.IsNullOrEmpty(str)) return str;
            ...
            for (int i = 0; i < str.Length; i++)
            {
                ...
                if (!valid.Any(p => string.Equals(chrStr, p, StringComparison.OrdinalIgnoreCase))) continue;
                strSB.Append(chrStr);
            }
            var newStr = strSB.ToString().TrimStart().TrimEnd();
            return newStr;
        }
        ...
        public HelicopterSpawn FindSpawn(string name, StringComparison comparison = StringComparison.Ordinal)
        {
            ...
            if (spawnsData?.HelicopterSpawns != null)
            {
                for(int i = 0; i < spawnsData.HelicopterSpawns.Count; i++)
                {
